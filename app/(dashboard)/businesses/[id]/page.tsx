...
import { db, businesses, businessDescriptions, backlinkPool, citationAccounts, sites } from "@/lib/db";
...
export default async function BusinessDetailPage({ params }: { params: { id: string } }) {
  ...
  const [descriptions, backlinks, accounts] = await Promise.all([
    db.select().from(businessDescriptions).where(eq(businessDescriptions.business_id, params.id)).orderBy(businessDescriptions.created_at),
    ...
    db.select({
      ...
      .where(eq(citationAccounts.business_id, params.id))
    ...
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-sm mt-0.5">{biz.owner_name}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 space-y-4">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex"><dt className="w-32 text-gray-400 flex-shrink-0">Website</dt>
                <dd><a href={biz.website} target="_blank" rel="noopener" className="text-primary hover:underline flex items-center gap-1">
                  {biz.website.replace(/^https?:\/\//, "")} <ExternalLink className="w-3 h-3" />
                </a></dd>
              </div>
              <div className="flex"><dt className="w-32 text-gray-400 flex-shrink-0">Address</dt>
                <dd className="text-gray-900">{biz.address_street}, {biz.address_city}, {biz.address_state} {biz.address_zip}</dd>
              </div>
              {biz.founding_year && (
                <div className="flex"><dt className="w-32 text-gray-400 flex-shrink-0">Founded</dt><dd className="text-gray-900">{biz.founding_year}</dd></div>
              )}
              <div className="flex"><dt className="w-32 text-gray-400 flex-shrink-0">Services</dt>
                <dd className="text-gray-900">{biz.service_categories.join(", ") || "—"}</dd>
              </div>
            </dl>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {approvedDescriptions.length} approved &middot; {descriptions.length} total
                </p>
              </div>
              <GenerateDescriptionsButton businessId={biz.id} />
            </div>
            {descriptions.length > 0 ? (
              <div className="space-y-3">
                ...
                {(descriptions as unknown as BusinessDescription[]).map((desc: BusinessDescription) => (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href={`/runs?business_id=${biz.id}`} className="text-xs text-primary hover:underline">View all runs</Link>
            </div>
            {accounts.length > 0 ? (
              <div className="space-y-2">
                {accounts.map((account) => (
                  <div key={account.id} className="flex items-center justify-between text-sm py-1.5">
                    <span className="text-gray-900">{account.site_name ?? "Unknown site"}</span>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {biz.gmail_connected_at ? (
              <div>
                <div className="flex items-center gap-2 text-green-600 text-sm mb-3">
                  <CheckCircle2 className="w-4 h-4" />
                  Connected
                </div>
                <p className="text-xs text-gray-400">CitationBot can automatically click email verification links.</p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-xs text-gray-400 mb-3">Connect Gmail to automate email verification during signups.</p>
                <ConnectGmailButton businessId={biz.id} />
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {biz.google_sheet_url ? (
              <a href={biz.google_sheet_url} target="_blank" rel="noopener"
                className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
                <ExternalLink className="w-3.5 h-3.5" /> Open Ledger
              </a>
            ) : (
              <p className="text-sm text-gray-400 italic">Sheet will be created after first successful citation signup.</p>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href={`/businesses/${biz.id}/edit#backlinks`} className="text-xs text-primary hover:underline">Edit</Link>
            </div>
            {backlinks.length > 0 ? (
              <div className="space-y-2">
                {(backlinks as unknown as BacklinkEntry[]).map((bl: BacklinkEntry) => (
