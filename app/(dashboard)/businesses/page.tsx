    ...
    <div className="p-8">
      ...
      {data.length > 0 ? (
            ...
            <Link key={business.id} href={`/businesses/${business.id}`}
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  {business.logo_url ? (
                    <Image src={business.logo_url} alt={business.name} width={40} height={40} className="rounded-lg object-cover" />
                  ) : (
                ...
                {business.gmail_connected_at ? (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-green-50 text-green-700 font-medium">Gmail connected</span>
                ) : (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-50 text-yellow-700 font-medium">No Gmail</span>
                )}
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 py-20 text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">Add your first business to start building citations.</p>
          <Link href="/businesses/new"
            className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors">
            <Plus className="w-4 h-4" />
            Add Business
