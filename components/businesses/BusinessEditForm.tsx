  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (validBacklinks.length > 0) {
      await supabase.from("backlink_pool").insert(
        validBacklinks.map(({ url, anchor_text }) => ({
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div id="backlinks" className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button type="button"
            onClick={() => setBacklinks((p) => [...p, { id: "", url: "", anchor_text: "" }])}
          ...
          {backlinks.map((bl, i) => (
            <div key={i} className="flex gap-2 items-start">
              <div className="flex-1 grid grid-cols-2 gap-2">
                <input value={bl.url} onChange={(e) => updateBacklink(i, "url", e.target.value)}
                  type="url" placeholder="https://..."
                  className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
                <input value={bl.anchor_text} onChange={(e) => updateBacklink(i, "anchor_text", e.target.value)}
                  placeholder="Anchor text"
                  className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              {backlinks.length > 1 && (
                <button type="button"
                  onClick={() => setBacklinks((p) => p.filter((_, idx) => idx !== i))}
